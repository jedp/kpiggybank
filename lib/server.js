...
app.post('/wsapi/interaction_data', function(req, res) {
  logger.info("POST from " + req.connection.remoteAddress);
  logger.info("POST body: " + JSON.stringify(req.body));
  db.saveAll(req.body, function(err) {
    if (err) {
      logger.error("POST crash: " + JSON.stringify(err));
      res.writeHead(400);
      res.write(err);
    } else {
      logger.info("Stored post from " + req.connection.remoteAddress);
      res.writeHead(201);
    }
    res.end();
  });
